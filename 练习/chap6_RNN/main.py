    ...
    for epoch in range(30):
        ...
        for batch in range(n_chunk):
            ...
            for index in range(BATCH_SIZE):
                ...
                if index == 0:
                    _, pre = torch.max(pre, dim=1)
                    #print('prediction', pre.data.cpu().tolist())  # 输出时移回CPU
...
def gen_poem(begin_word):
    ...
    word = begin_word
    max_length=30
    while len(poem) < max_length and word != end_token:
        ...
        poem += word
    # 去掉末尾的 end_token
    if poem.endswith(end_token):
        poem = poem[:-1]
    return poem

def gen_poem_with_GPU(begin_word):
    poems_vector, word_int_map, vocabularies = process_poems1('./poems.txt')
    # 加载模型到GPU
    ...
    rnn_model.load_state_dict(torch.load('./poem_generator_rnn', map_location=device))  # 加载时指定设备
    ...
    while word != end_token:
        ...
        output = rnn_model(input, is_test=True)
        word = to_word(output.data.cpu().numpy()[-1], vocabularies)  # 输出移回CPU处理
        poem += word
        if len(poem) > 30:
            break
    return poem
...
if torch.cuda.is_available():
    ...
    pretty_print_poem(gen_poem_with_GPU("日"))
    pretty_print_poem(gen_poem_with_GPU("红"))
    pretty_print_poem(gen_poem_with_GPU("山"))
    pretty_print_poem(gen_poem_with_GPU("夜"))
    pretty_print_poem(gen_poem_with_GPU("湖"))
    pretty_print_poem(gen_poem_with_GPU("湖"))
    pretty_print_poem(gen_poem_with_GPU("湖"))
    pretty_print_poem(gen_poem_with_GPU("君"))
else:
